...
let ffmpegx: FFmpeg<FFmpegConfigurationGPLExtended> | null = null;

export function initFFmpeg() {
  if (ffmpegx) {
    return;
  }
  ffmpegx = new FFmpeg<FFmpegConfigurationGPLExtended>({
    config: 'gpl-extended',
  });
}
...
async function writeCanvasToFile(canvas: HTMLCanvasElement, filename: string): Promise<void> {
  return new Promise((resolve, reject) => {
    canvas.toBlob(async blob => {
      if (blob) {
        await ffmpegx!.writeFile(filename, blob);
        resolve();
      } else {
...
export async function createVideoWithImages(w: number, h: number, fps: number, scenes: HTMLCanvasElement[], reportProgress: (p:number) => void): Promise<string> {
  console.log('createVideoWithImages', w, h, fps);

  const ffmpeg = ffmpegx!;

  // 各画像をファイルシステムに書き込む
  let progress = 0.5;
  for (let i = 0; i < scenes.length; i++) {
    const scene = scenes[i];
    await writeCanvasToFile(scene, `image${i}.png`);
